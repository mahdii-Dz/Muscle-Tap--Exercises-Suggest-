...
function ProgressBtn() {
    ...
    return (
        <div className='flex justify-between rounded-full overflow-clip z-50 gap-4 p-2 border border-slate-500/50 items-center w-full max-w-[calc(100%-2rem)] sm:max-w-[calc(45%-2rem)] fixed sm:bottom-20 bottom-8 sm:right-1/5 right-1/2 translate-1/2 sm:translate-0 bg-gray-900'>
            ...
            <button onClick={() => handlePageNum('next')} className={`w-1/2 h-12 flex justify-center gap-2 items-center font-semibold rounded-full hover:scale-[98%] transition duration-200 hover:bg-primary-hover-1 cursor-pointer bg-primary-1 text-white ${muscles?.length === 0}`}>
                {
                    pageNum === 3
                        ? <span>Finish</span>
                        : pageNum === 4
